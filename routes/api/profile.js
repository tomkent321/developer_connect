...
// @desc    Get a list of all the profiles
// @access   Public
router.get("/all", (req, res) => {
  ...
  Profile.find()
    ...
    .catch(err =>
      res.status(404).json({ profiles: "There are no profiles to list" })
    );
});
...
router.post(
  ...
  (req, res) => {
    ...
    // Skills - Split CSV in to an array
    if (typeof req.body.skills !== "undefined") {
      profileFields.skills = req.body.skills.split(",");
    }
    //Social...since it is in its own object, the input must be initialized as and empty object
